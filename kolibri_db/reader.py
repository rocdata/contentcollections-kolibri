#!/usr/bin/env python
"""
Utilities for downloding Kolibri databases and printing topic trees.

### Library usage
>>> from kolibri_db.reader import download_db_file, dbconnect, get_tree
>>> db_file_path = download_db_file(channel_id, server='production', update=True)
>>> conn = dbconnect(db_file_path)
>>> kolibri_tree = get_tree(conn)
The result `kolibri_tree` (dict) is a complete tree of the entire channel.
Use `export_kolibri_json_tree` to save the channel tree as a JSON file.

### CLI usage
This script is also usable as command line tool:
    ./reader.py --channel_id=95a52b386f2c485cb97dd60901674a98
or to get the same result as HTML (assuming you have `pandoc` installed):
    ./reader.py --channel_id=95a52b386f2c485cb97dd60901674a98 --htmlexport
...
if __name__ == '__main__':
    ...
    # DUMP CHANNEL JSON
    export_kolibri_json_tree(db_file_path=db_file_path)
